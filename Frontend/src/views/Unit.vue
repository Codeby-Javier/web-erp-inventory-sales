    ...
    <div class="page-header d-print-none mb-4">
      <div class="row g-2 align-items-center">
        <div class="col">
          ...
          <h2 class="page-title">Satuan Produk (UoM)</h2>
        </div>
          ...
          <table class="table card-table table-vcenter text-nowrap datatable">
            ...
            <tbody>
              ...
              <tr v-else-if="items.length === 0">
                <td colspan="5" class="text-center text-muted py-4">Data satuan kosong.</td>
              </tr>
...
const formFields = [
  { key: 'name', label: 'Nama Satuan (Singkat)', type: 'text', required: true },
  { key: 'name_plural', label: 'Nama Jamak (Opsional)', type: 'text', required: false },
...
const deleteItem = async (item) => {
  if (confirm(`Apakah Anda yakin ingin menghapus satuan "${item.name}"?`)) {
    try {
      await api.delete(`/crud/quantity_units/${item.id}`)
      fetchData()
    } catch (error) {
      alert(error.response?.data?.message || 'Gagal menghapus data. Data mungkin digunakan di tabel lain.')
    }
  }
}
